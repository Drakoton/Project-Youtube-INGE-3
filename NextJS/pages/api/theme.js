...
export default function handler(req, res) {
  ...
  const scriptPath = path.join(process.cwd(), 'scripts', 'sentiment_analysis_matrice.py');
  ...
  python.on('close', (code) => {
    ...
    try {
      ...
      if (!data.video_themes) {
        return res.status(500).json({ error: 'video_themes is missing in the response' });
      }
      return res.status(200).json({ ...data, video_themes: data.video_themes });
    } catch (err) {
